...
case "${PLATFORM}" in
  ...
  "retropie"|"setup") exit 0 ;; # fbterm does not like splash
esac
...
if [ "${ACTION_TYPE}" = "intro" ] || [ "${ACTION_TYPE}" = "exit" ]; then
  ...
  # Exit overrides from emuelec.conf
  ...
  if [[ "${MODE}" == *"x"* ]]; then
    ...
    [ -f "/storage/.config/splash/loading-game-std.mp4" ] && GAMELOADINGSPLASH="/storage/.config/splash/loading-game-std.mp4"
  fi

  # Extended game loading logic
  ...
  if [ -z "${SPLASH}" ]; then
    ...
    REALSPL="${SPLNAME#*\"}"; REALSPL="${REALSPL%\"*}"

    [ -n "${ROMNAME}" ] && SPLASH1="$(find ${SPLASHDIR}/${PLATFORM} -iname "${ROMNAME}*.png" -maxdepth 1 | sort -V | head -n 1)"
    [ -n "${ROMNAME}" ] && SPLASHVID1="$(find ${SPLASHDIR}/${PLATFORM} -iname "${ROMNAME}*.mp4" -maxdepth 1 | sort -V | head -n 1)"
    [ -n "${REALSPL}" ] && SPLASH2="$(find ${SPLASHDIR}/${PLATFORM} -iname "${REALSPL}*.png" -maxdepth 1 | sort -V | head -n 1)"
    [ -n "${REALSPL}" ] && SPLASHVID2="$(find ${SPLASHDIR}/${PLATFORM} -iname "${REALSPL}*.mp4" -maxdepth 1 | sort -V | head -n 1)"
...
# Devices without splash support
SS_DEVICE=0
...
# Non-intro or intro depending on novideo flag
if [[ -f "/storage/.config/emuelec/configs/novideo" ]] && [[ "${VIDEO}" != "1" ]]; then
  # All splashes except boot video
  if [ "${SS_DEVICE}" = "1" ]; then
    # mpv branch
    if [[ "${EXTENSION,,}" = "mp4" ]]; then
      mpv --fullscreen --no-keepaspect --vf=lavfi="${FILTER_FILL}" --quiet --really-quiet --no-audio "${SPLASH}" > /dev/null 2>&1
    else
      mpv --fullscreen --no-keepaspect --vf=lavfi="${FILTER_FILL}" --image-display-duration=3 --quiet --really-quiet --no-audio "${SPLASH}" > /dev/null 2>&1
    fi
  else
    # ffplay branch
    if [[ "${EXTENSION,,}" = "mp4" ]]; then
  ...
  if [ "${SS_DEVICE}" = "1" ]; then
    mpv --fullscreen --no-keepaspect --vf=lavfi="${FILTER_FILL}" --quiet --really-quiet "${SPLASH}" > /dev/null 2>&1
  else
    ffplay -fs -autoexit -loglevel error -nostats -vf "${FILTER_FILL}" "${SPLASH}" > /dev/null 2>&1
  fi
...
# Wait according to ee_splash.delay
